...
import com.nari.qrystat.colldataanalyse.TGLosePowerStatManager;
 ...
 * @author 姜炜超
 */
public class TGLosePowerStatManagerImpl implements TGLosePowerStatManager {
	
	private TGLosePowerStatJdbcDao tgLosePowerStatJdbcDao; 
	 
	public void setTgLosePowerStatJdbcDao(
			TGLosePowerStatJdbcDao tgLosePowerStatJdbcDao) {
		this.tgLosePowerStatJdbcDao = tgLosePowerStatJdbcDao;
	}
	...
	public Page<LosePowerStatDto> findLosePowerStat(String nodeId, String nodeType, String orgType, 
		...
		if(null == statFlag || null == startDate || null == endDate || null == nodeId){
			return new Page<LosePowerStatDto>();
		}
		...
		try{
			psc = tgLosePowerStatJdbcDao.findLosePowerStat(nodeId, nodeType, orgType, statFlag, 
					startD, endD, start, limit, pSysUser);
		}catch(DataAccessException e) {
			throw new DBAccessException(BaseException.processDBException(e));
		} catch (Exception e) {
			throw new ServiceException(ExceptionConstants.QYE_TGLOSEPOWERSTATINFO);
		}
