...
import com.nari.qrystat.colldataanalyse.LosePowerStatManager;
 ...
 * @author 姜炜超
 */
public class LosePowerStatManagerImpl implements LosePowerStatManager{
    private LosePowerStatJdbcDao losePowerStatJdbcDao;
    private Logger logger = Logger.getLogger(this.getClass());
	public void setLosePowerStatJdbcDao(LosePowerStatJdbcDao losePowerStatJdbcDao) {
		this.losePowerStatJdbcDao = losePowerStatJdbcDao;
	}
	
	/**
	 * 根据条件查询时间段内台区用电损耗查询信息，主页面grid显示内容
	 * @param nodeId 节点id
	 * @param nodeType 节点类型
	...
	public Page<LosePowerStatDto> findLPSInfoByCond(String nodeId, String nodeType,String orgType,  
		...
		Page<LosePowerStatDto> psc = null;//定义返回分页对象
		Integer stratD = 0;
		Integer endD = 0;
		...
		try {
			...
			psc = losePowerStatJdbcDao.findLPSInfoByCond(nodeId, nodeType, orgType, stratD, endD, start, limit, pSysUser);
			
			if(null == psc){
				psc = new Page<LosePowerStatDto>();
			}else{
				if(null != psc.getResult() && 0 < psc.getResult().size()){
				    for(int i = 0 ; i < psc.getResult().size(); i++){
				    	LosePowerStatDto dto = (LosePowerStatDto)psc.getResult().get(i);
				    	dto.setStartDate(startDate);//仅塞一下，页面不做处理
				    	dto.setEndDate(endDate);//仅塞一下，页面不做处理
				    	
				    	if(null != dto.getPpq() && null != dto.getSpq()){
				    	    dto.setLpq(ArithmeticUtil.sub(dto.getPpq(), dto.getSpq()));//损失电量= 合计供电量-合计售电量
				    	}else{
				    		dto.setLpq(null);
				    	}
				    	
				    	if(null != dto.getLpq() && 0 != dto.getPpq()){
				    		//总电量不为0，则计算出有损线损率=（损失电量/合计供电量）*100%
				    		dto.setLlr(ArithmeticUtil.round(100*ArithmeticUtil.div(dto.getLpq(), dto.getPpq(),4),2));
				    	}else{
				    		dto.setLlr(null);
				    	}
			...
			throw new ServiceException(ExceptionConstants.QYE_LOSEPOWERSTATINFO);
		}
	...
	public Page<LosePowerStatReadBean> findLPSReadInfo(String tgId, Date startDate, String flag, String ioValue,
		...
		if(null == tgId || null == startDate || null ==endDate || null == ioValue){
			return new Page<LosePowerStatReadBean>();
		}
		try{
//			if(flag){
			    return losePowerStatJdbcDao.findLPSReadSuccInfo(tgId, DateUtil.dateToString(startDate), 
					ioValue,flag, DateUtil.dateToString(endDate), start, limit, pSysUser);
//			}else{
//				return losePowerStatJdbcDao.findLPSReadFailInfo(tgId, DateUtil.dateToString(startDate), 
...
//			}
	...
	public void recallData(Boolean checkAll, String tgId, Date startDate, Boolean flag, String ioValue, 
		...
		try {
			...
			if(checkAll){
				...
				list = losePowerStatJdbcDao.findTmnlMissingInfoByAll(tgId, DateUtil.dateToString(startDate), ioValue, 
						DateUtil.dateToString(endDate));
			}else{
				list = losePowerStatJdbcDao.findTmnlMissingInfoByANList(tmnlAssetNoList, DateUtil.dateToString(startDate), 
						DateUtil.dateToString(endDate));
			}
			...
			for( int i = 0; i < list.size(); i++){
				...
				//rtList可能为null，以至于下面的方法异常recallData
				if (rtList == null) {
					throw new ServiceException("补召任务下发出错");
				}
			}
		} catch(DataAccessException e) {
			throw new DBAccessException(BaseException.processDBException(e));
	...
	public void recalcData(String orgNo, Date startDate, Date endDate)throws Exception{
		...
		for( int i = 0 ; i <= interval; i++){
			losePowerStatJdbcDao.recalcData(orgNo, DateUtil.afterDateNC(startDate,i));
		}
	}

	@Override
	public List<Map> findChartData(Integer tgId, Date start, Date end) throws ServiceException {
		try {
			return losePowerStatJdbcDao.findChartData(tgId, start, end);
		} catch (Exception e) {
