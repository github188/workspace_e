...
public class DataFetchJdbcDaoImpl extends JdbcBaseDAOImpl implements
	...
	public Map<String, List> findFrmToAsset(String tmnlAssetNo) {
		String sql = "select dlc.asset_no as fmr_asset_no, e.asset_no,decode(cc.fmr_asset_no,null,1,cc.tmnl_asset_no,1,0) as is_tmnl\n"
				+ // is_tmnl确定是不是
				"  from e_data_mp e\n"
		...
		getJdbcTemplate().query(sql, new Object[] { tmnlAssetNo },
				new RowMapper() {
					@Override
					public Object mapRow(ResultSet rs, int rowNum)
							throws SQLException {
	...
	@Override
	public boolean updateOnOffStatus(String meterID, String dueStatus,
			String switchStatus) throws DBAccessException {
		if (dueStatus.equals("JF")&& switchStatus.equals("ON")) {
			String updateSql = "update sea_userqfdata_cur s"
					+ " set s.qf_flag = 'ZC' ,s.on_off_flag = ? \n "
					+ " where s.meterid = ?";
			int i = getJdbcTemplate().update(updateSql,
					new Object[] { switchStatus, meterID });
			if (i != 0) {
				return true;
			}
		} else {
