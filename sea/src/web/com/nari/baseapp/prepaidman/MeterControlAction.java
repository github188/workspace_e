...
public class MeterControlAction extends BaseAction {
	...
	private List<MeterControlInfoBean> meterList;
	...
	public String queryTmnlList() throws Exception {
		...
		PSysUser userInfo = (PSysUser) this.getSession().getAttribute(
				"pSysUser");
		tmnlList = prePaidManager.queryTermList(nodeType, nodeValue, userInfo,
				protocolCode);
		this.logger.debug("----终端查询结束----");
		return SUCCESS;
	}
	// ---------------------------四川盘龙电表通断--------------------------
	public String queryMeterList() throws Exception {
		this.logger.debug("----电表查询开始----");
		// 获取操作员信息
		PSysUser userInfo = (PSysUser) this.getSession().getAttribute(
				"pSysUser");
		meterList = prePaidManager.queryMeterList(nodeType, nodeValue,
				userInfo, protocolCode);
		this.logger.debug("----电表查询结束----");
		return SUCCESS;
	}

	public List<MeterControlInfoBean> getMeterList() {
		return meterList;
	}

	public void setMeterList(List<MeterControlInfoBean> meterList) {
	...
	public String queryTmnlArrList() throws Exception {
		...
		PSysUser userInfo = (PSysUser) this.getSession().getAttribute(
				"pSysUser");
		tmnlList = prePaidManager.queryTmnlArrList(nodeType, nodeValue,
				userInfo, protocolCode);
	...
	public String sendPagePara() {
		...
		FetchInfoBean[] bpItem = (FetchInfoBean[]) JSONSerializer.toJava(
				jsonArray.getJSONArray(0), jsonConfig);
		...
		for (int n = 0; n < meterList.size(); n++) {
			...
			// controlType 是页面单选框值，01 告警 02 告警解除 03 合闸 04 分闸 05 保电 06 保电解除
			// (还会增加)
			...
			if (n == 0) {
				...
				if (meterList.size() == 1) {// 如果前台终端List长度为1，直接进行下发
					taskDeal.qstTermParamTaskResult(tmnlAssetNoList,
							FrontConstant.TRANS_DATA, paramList, meterInfoList);
					logger.debug("--调用接口--");
				}
				continue;
			}
		...
		if (null != list && 0 < list.size()) {
			for (int i = 0; i < list.size(); i++) {
				...
				lOpTmnlLog.setOpButton(getOpButtonByCtrlType(controlType));// 转发按钮记录:
				// 01
				// 告警，
				// 02
				// 拉闸
				...
				if (tmnlStatus == 3) {
					// 根据终端资产编号取得终端下的电表列表
					List<Map<String, String>> meterInTmnlList = meterInTmnlList(
							meterList, tmnlAssetNo);
					...
					for (int j = 0; j < meterInTmnlList.size(); j++) {
						...
						ResponseItem item = findResItemByMeterId(meterMap
								.get("meterId"), resItems);

						// 错误码，当item为null时置-1
						short errorCode = item == null ? (short) -1 : item
								.getErrorCode();
						// errorCode大于0时 全部是成功
						if (errorCode >= 0) {
							errorCode = (short) 1;
						...
						// 记录下发状态
						tmnlStatusMap.put(tmnlAssetNo + meterId, Short
								.toString(errorCode));
					}
				} else {
					// 根据终端资产编号取得终端下的电表列表
					...
					for (int j = 0; j < meterInTmnlList.size(); j++) {
						...
						lOpTmnlLog.setOpObjNo(Long
								.parseLong(map.get("meterId")));
						// 将所有数据项记录操作日志
						for (int k = 0; k < items.size(); k++) {
							Item it = items.get(k);
							lOpTmnlLog.setProtItemNo(it.getCode());
						...
						// 记录下发状态
						tmnlStatusMap.put(tmnlAssetNo + map.get("meterId"),
								"-1");
					}
				}
			}
	 ...
	 * @return
	 */
	protected Map<String, String> getMeterMapByMeterId(
			List<Map<String, String>> meterList, String meterId) {
	...
	private List<FetchInfoBean> getTerminalInfo(String tmnlAssetNo, short pn) {
		...
		try {
			if ("22".equals(this.dataType) || "24".equals(this.dataType)) {
				short[] pnTotal = { pn };
				rs = terminalParaSetManager.getTmnlMpList(this.proNO,
						tmnlAssetNo, pnTotal);
			} else {
				rs = terminalParaSetManager.getTerminalList(this.proNO,
						tmnlAssetNo);
			}
		} catch (Exception e) {
