...
var commMode= new Ext.form.ComboBox( {
	...
	name : 'areaDatRepTIRScommMode',
	id : 'areaDatRepTIRScommMode',
	store : commModeStore,
	// mode : 'remote',
	fieldLabel : '通信方式',
	triggerAction : 'all',
...
reportParamStore.on('load', function() {
	Ext.getCmp('areaDatRepTIRSGrid').getSelectionModel().selectAll();
});
	...
	items : [
			...
			},
			{
				...
				items : [ {
					...
					readOnly : false,
					labelSeparator : '',
					name : 'areaDatRepTIRSDescription',
					anchor : '100%',
...
function templateManager() {
	Ext.getCmp('areaDatRepTIRSGrid').getStore().eachAll(function(key,v){
	    params.push(v["orgNo"]);});
	var consNoList = '';
	...
	items : [ // reportParamsPanel, bottomParamGridPanel]//,
			...
			reportParamsPanel,
			{
				...
				items : [ {
					xtype : 'grid',
					mode : 'remote',
					id : 'areaDatRepTIRSGrid',
					name : 'areaDatRepTIRSGrid',
					// ds : reportParamStore,
					region : 'south',
					cm : reportParamCm,
				...
				tbar : [
						{
							xtype : 'label',
							html : "<font font-weight:bold;>终端安装及运行统计</font>"
						},'-',{
							xtype : "checkbox",
							boxLabel : "全选",
							id : "areaDatRepTIRS_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
						...
						},
						{
							...
							width : 30,
							handler : function() {
							if (Ext.getCmp('areaDatRepTIRS_selectAll').checked) {
	...
	processClick : function(p, node, e) {
		...
		if (type == 'org') {
			...
			if (orgType == '04') {
				...
				Ext.getCmp('areaDatRepTIRSGrid').getSelectionModel()
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("areaDatRepTIRSStartTime").getRawValue();
	var endTime = Ext.getCmp("areaDatRepTIRSEndTime").getRawValue();
