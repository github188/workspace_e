...
var commMode= new Ext.form.ComboBox( {
	hideOnSelect : false,
	displayField : 'commMode',
	valueField : 'commModeCode',
	name : 'areaDatRepTRDScommMode',
	id : 'areaDatRepTRDScommMode',
	store : commModeStore,
	// mode : 'remote',
	fieldLabel : '通信方式',
	triggerAction : 'all',
...
reportParamStore.on('load', function() {
	Ext.getCmp('areaDatRepTRDSGrid').getSelectionModel().selectAll();
});
if(defBool==1){
	Ext.Ajax.request({
	...
	items : [
			...
			},
			{
				...
				items : [ {
					...
					readOnly : false,
					labelSeparator : '',
					name : 'areaDatRepTRDSDescription',
					anchor : '100%',
...
function templateManager() {
	Ext.getCmp('areaDatRepTRDSGrid').getStore().eachAll(function(key,v){
	    params.push(v["orgNo"]);});
	var consNoList = '';
	...
	items : [ // reportParamsPanel, bottomParamGridPanel]//,
			...
			reportParamsPanel,
			{
				...
				items : [ {
					xtype : 'grid',
					mode : 'remote',
					id : 'areaDatRepTRDSGrid',
					name : 'areaDatRepTRDSGrid',
					// ds : reportParamStore,
					region : 'south',
					cm : reportParamCm,
				...
				tbar : [
						{
							xtype : 'label',
							html : "<font font-weight:bold;>终端运行调试状态统计</font>"
						},'-',{
							xtype : "checkbox",
							boxLabel : "全选",
							id : "areaDatRepTRDS_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
						...
						},
						{
							...
							width : 30,
							handler : function() {
							if (Ext.getCmp('areaDatRepTRDS_selectAll').checked) {
	...
	processClick : function(p, node, e) {
		...
		if (type == 'org') {
			...
			if (orgType == '04') {
				...
				Ext.getCmp('areaDatRepTRDSGrid').getSelectionModel()
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("areaDatRepTRDSStartTime").getRawValue();
	var endTime = Ext.getCmp("areaDatRepTRDSEndTime").getRawValue();
