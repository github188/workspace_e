...
var voltDegreeCombox = new Ext.form.ComboBox({
				...
				name:'areaDatRepCTISvoltDegree',
				id:'areaDatRepCTISvoltDegree',
...
reportParamStore.on('load', function() {
	Ext.getCmp('areaDatRepCTISGrid').getSelectionModel().selectAll();
});
if(defBool==1){
	Ext.Ajax.request({
	...
	items : [
			...
			},
			{
				...
				items : [ {
					...
					readOnly : false,
					labelSeparator : '',
					name : 'areaDatRepCTISDescription',
					anchor : '100%',
...
function templateManager() {
	Ext.getCmp('areaDatRepCTISGrid').getStore().eachAll(function(key,v){
	    params.push(v["orgNo"]);});
	var consNoList = '';
	...
	items : [ // reportParamsPanel, bottomParamGridPanel]//,
			...
			reportParamsPanel,
			{
				...
				items : [ {
					xtype : 'grid',
					mode : 'remote',
					id : 'areaDatRepCTISGrid',
					name : 'areaDatRepCTISGrid',
					// ds : reportParamStore,
					region : 'south',
					cm : reportParamCm,
				...
				tbar : [
						{
							xtype : 'label',
							html : "<font font-weight:bold;>采集终端安装统计报表</font>"
						},'-',{
							xtype : "checkbox",
							boxLabel : "全选",
							id : "areaDatRepCTIS_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
						...
						},
						{
							...
							width : 30,
							handler : function() {
							if (Ext.getCmp('areaDatRepCTIS_selectAll').checked) {
	...
	processClick : function(p, node, e) {
		...
		if (type == 'org') {
			...
			if (orgType == '04') {
				...
				Ext.getCmp('areaDatRepCTISGrid').getSelectionModel()
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("areaDatRepCTISStartTime").getRawValue();
	var endTime = Ext.getCmp("areaDatRepCTISEndTime").getRawValue();
