...
var voltDegreeCombox = new Ext.form.ComboBox({
				hideOnSelect : false,
				displayField : 'voltDegree',
				valueField : 'voltDegree',
				name:'areaDatRepVDERvoltDegree',
				id:'areaDatRepVDERvoltDegree',
...
reportParamStore.on('load', function() {
	Ext.getCmp('areaDatRepVDERGrid').getSelectionModel().selectAll();
});
	...
	items : [
			...
			},
			{
				...
				items : [ {
					...
					readOnly : false,
					labelSeparator : '',
					name : 'areaDatRepVDERDescription',
					anchor : '100%',
...
function templateManager() {
	Ext.getCmp('areaDatRepVDERGrid').getStore().eachAll(function(key,v){
	    params.push(v["orgNo"]);});
	var consNoList = '';
	...
	items : [ // reportParamsPanel, bottomParamGridPanel]//,
			...
			reportParamsPanel,
			{
				...
				items : [ {
					xtype : 'grid',
					mode : 'remote',
					id : 'areaDatRepVDERGrid',
					name : 'areaDatRepVDERGrid',
					// ds : reportParamStore,
					region : 'south',
					cm : reportParamCm,
				...
				tbar : [
						{
							xtype : 'label',
							html : "<font font-weight:bold;>电压等级采集电量报表</font>"
						},'-',{
							xtype : "checkbox",
							boxLabel : "全选",
							id : "areaDatRepVDER_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
						...
						},
						{
							...
							width : 30,
							handler : function() {
							if (Ext.getCmp('areaDatRepVDER_selectAll').checked) {
	...
	processClick : function(p, node, e) {
		...
		if (type == 'org') {
			...
			if (orgType == '04') {
				...
				Ext.getCmp('areaDatRepVDERGrid').getSelectionModel()
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("areaDatRepVDERStartTime").getRawValue();
	var endTime = Ext.getCmp("areaDatRepVDEREndTime").getRawValue();
