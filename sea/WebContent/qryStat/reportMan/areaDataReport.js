...
//				proxy:new Ext.data.MemoryProxy(),
		...
		reportStore.baseParams = {reportType :"地区数据报表"};
		reportStore.load();
...
/**------------------------------------综合操作----------------------------------- */
	...
	var previousProcess = function(btn) {
		if (reportManCard.layout.activeItem == reportTypePanel) {
			...
			// } else if (reportManCard.layout.activeItem == remoteDebugPanel) {
			// reportManCard.layout.setActiveItem(4);
			// } else {
		}
	};
	// 下一步按钮处理方法
...
//	nextButton.addEvents('click',nextProcess);
	...
	previousButton.hide();
	renderModel(reportManCard, '地区数据报表');
});