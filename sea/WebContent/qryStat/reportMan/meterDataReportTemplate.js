...
Ext.onReady(function() {
		...
		reportStore = new Ext.data.JsonStore({
					...
					fields : [ 'templateId', 'reportId','rawReportType', 'rawReportName',
							'reportType', 'reportName', 'consNoList',
							'reportParam' ],
					root : 'repTemList',
					totalProperty : "reportCount"
				});
		reportStore.baseParams = {
			reportType : "自定义电表数据报表"
		};
		reportStore.load();
		...
		reportTypeGrid = new Ext.grid.GridPanel( {
			...
			tbar : [
					{
						xtype : 'label',
						html : "<font font-weight:bold;>自定义电表数据报表</font>"
					},
					'->',
					{
						text : '删除模板',
		...
		nextProcess = function() {
			if (reportManCard.layout.activeItem == reportTypePanel) {
				...
				if ((Ext.isEmpty(reportTypeGrid.getSelectionModel()
						.getSelected()))) {
		       ...
		       var reportName = reportTypeGrid.getSelectionModel().getSelected()
				.get('rawReportName');
//				var reportName = reportTypeGrid.getSelectionModel()
//						.getSelected().get('reportName');
				reportId = reportTypeGrid.getSelectionModel().getSelected()
						.get('reportId');
				reportManCard.remove(reportParamsPanelSecond);
				Ext.Ajax.request( {
