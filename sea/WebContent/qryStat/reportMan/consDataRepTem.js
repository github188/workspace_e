...
Ext.onReady(function() {
	...
	var reportStore = new Ext.data.JsonStore({
		...
		reportStore.baseParams = {reportType :"自定义用户数据报表"};
		reportStore.load();
	...
	reportTypeGrid = new Ext.grid.GridPanel({
				...
				tbar : [{
								xtype : 'label',
								html : "<font font-weight:bold;>自定义用户数据报表</font>"
							}]
			});
...
/**------------------------------------综合操作----------------------------------- */
	...
	nextProcess = function() {
		if (reportManCard.layout.activeItem == reportTypePanel) {
			...
			Ext.Ajax.request({
					...
					success : function(response) {
						...
						scriptConent=scriptConent.replace(/consDatRep/g,'consDatRepTem');
						scriptConent=scriptConent.replace('saveConsNoList','updateConsNoList');
						eval(scriptConent);	
		...
		previousButton.hide();
		renderModel(reportManCard, '自定义用户数据报表');
	});