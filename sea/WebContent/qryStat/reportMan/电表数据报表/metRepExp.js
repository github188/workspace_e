...
//			Ext.getCmp('metDatRepMREGrid').getSelectionModel().selectAll();
//		});
var params=[];		
...
function templateManager() {
	Ext.getCmp('metDatRepMREGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("metDatRepMREType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("metDatRepMREName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("metDatRepMREDescription").getValue()
				},
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>电表数据导出</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "metDatRepMRE_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('metDatRepMRE_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('metDatRepMREGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("metDatRepMREStartTime").getRawValue();
	var endTime = Ext.getCmp("metDatRepMREEndTime").getRawValue();
