...
//			Ext.getCmp('metDatRepMDRGrid').getSelectionModel().selectAll();
//		});
		var params=[];		
...
var templateInfoWindow = new Ext.Window({
			...
			autoScroll : true,
			modal : true,
			plain : true,// 璁剧疆鑳屾櫙棰滆壊
			resizable : false,// 涓嶅彲绉诲姩
			buttonAlign : "center",// 鎸夐挳鐨勪綅缃?
			closeAction : "hide",
...
function templateManager() {
	Ext.getCmp('metDatRepMDRGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("metDatRepMDRType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("metDatRepMDRName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("metDatRepMDRDescription").getValue()
				},
				success : function(response, opts) {
					Ext.Msg.alert('提示信息', '保存成功！');
				},
				failure : function(response, opts) {
					Ext.Msg.alert('提示信息', '模板保存失败！');
				},
				waitTitle : '提示信息',
				waitMsg : '请稍候.......'
			});
				...
				tbar : [{
							...
							handler : function() {
								...
								if (recs.length = 0 || Ext.isEmpty(recs)) {
									Ext.MessageBox.alert("提示信息", "请选择报表名称！");
									return;
								} else {
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('metDatRepMDR_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('metDatRepMDRGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("metDatRepMDRStartTime").getRawValue();
	var endTime = Ext.getCmp("metDatRepMDREndTime").getRawValue();
	var consList = new Array();
	var recs = userTerminal.getSelections();
	if (startTime <= endTime) {
		timeSelect = true;
	} else {
		Ext.MessageBox.alert("提示信息", "开始时间必须小于结束时间!");
		timeSelect = false
		return;
	}
	if (null == recs || 0 == recs.length) {
		Ext.MessageBox.alert("提示信息", "请选择用户！");
		dataSelect = false;
		return;
	} else {
		dataSelect = true;
