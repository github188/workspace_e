...
reportParamStore.on('load', function() {
			Ext.getCmp('staRepSDFGrid').getSelectionModel().selectAll();
		});
var params=[];
...
function templateManager() {
	Ext.getCmp('staRepSDFGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("staRepSDFType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("staRepSDFName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("staRepSDFDescription").getValue()
				},
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>抄读失败电表报表</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "staRepSDF_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('staRepSDF_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('staRepSDFGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("staRepSDFStartTime").getRawValue();
	var endTime = Ext.getCmp("staRepSDFEndTime").getRawValue();
