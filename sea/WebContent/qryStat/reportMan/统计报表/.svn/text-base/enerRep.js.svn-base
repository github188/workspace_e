...
reportParamStore.on('load', function() {
			Ext.getCmp('staRepEPGrid').getSelectionModel().selectAll();
		});
var params=[];
...
function templateManager() {
	Ext.getCmp('staRepEPGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("staRepEPType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("staRepEPName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("staRepEPDescription").getValue()
				},
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>电量报表</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "staRepEP_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('staRepEP_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('staRepEPGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("staRepEPStartTime").getRawValue();
	var endTime = Ext.getCmp("staRepEPEndTime").getRawValue();
