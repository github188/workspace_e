...
reportParamStore.on('load', function() {
			Ext.getCmp('staRepSEBSGrid').getSelectionModel().selectAll();
		});
var params=[];
...
function templateManager() {
	Ext.getCmp('staRepSEBSGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("staRepSEBSType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("staRepSEBSName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("staRepSEBSDescription").getValue()
				},
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>预购电工单汇总</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "staRepSEBS_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('staRepSEBS_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('staRepSEBSGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("staRepSEBSStartTime").getRawValue();
	var endTime = Ext.getCmp("staRepSEBSEndTime").getRawValue();
