...
reportParamStore.on('load', function() {
			Ext.getCmp('staRepPRGrid').getSelectionModel().selectAll();
		});
var params=[];
...
function templateManager() {
	Ext.getCmp('staRepPRGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("staRepPRType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("staRepPRName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("staRepPRDescription").getValue()
				},
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>功率报表</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "staRepPR_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('staRepPR_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('staRepPRGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("staRepPRStartTime").getRawValue();
	var endTime = Ext.getCmp("staRepPREndTime").getRawValue();
