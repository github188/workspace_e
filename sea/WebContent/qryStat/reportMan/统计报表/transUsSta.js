...
reportParamStore.on('load', function() {
			Ext.getCmp('staRepTUSGrid').getSelectionModel().selectAll();
		});
var params=[];
...
function templateManager() {
	Ext.getCmp('staRepTUSGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("staRepTUSType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("staRepTUSName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("staRepTUSDescription").getValue()
				},
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>变压器利用率统计</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "staRepTUS_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('staRepTUS_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('staRepTUSGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("staRepTUSStartTime").getRawValue();
	var endTime = Ext.getCmp("staRepTUSEndTime").getRawValue();
