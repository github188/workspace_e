...
reportParamStore.on('load', function() {
			Ext.getCmp('consDatRepCMEDGrid').getSelectionModel().selectAll();
		});
		var params=[];
...
var reportTemplatInfo = new Ext.form.FormPanel({
			...
			items : [{
						...
						items : [{
							...
							readOnly : false,
							labelSeparator : '',
							name : 'consDatRepCMEDDescription',
							anchor : '100%',
...
function templateManager() {
	Ext.getCmp('consDatRepCMEDGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("consDatRepCMEDType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("consDatRepCMEDName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("consDatRepCMEDDescription").getValue()
				},
...
reportParamsPanelSecond = new Ext.Panel({
			...
			items : [reportParamsPanel, {
				...
				items : [{
							xtype : 'grid',
							mode : 'remote',
							id : 'consDatRepCMEDGrid',
							name : 'consDatRepCMEDGrid',
							// ds : reportParamStore,
							region : 'south',
							cm : reportParamCm,
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>用户月电量明细</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "consDatRepCMED_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('consDatRepCMED_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('consDatRepCMEDGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("consDatRepCMEDStartTime").getRawValue();
	var endTime = Ext.getCmp("consDatRepCMEDEndTime").getRawValue();
