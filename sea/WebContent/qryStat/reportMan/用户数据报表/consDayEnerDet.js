...
reportParamStore.on('load', function() {
			Ext.getCmp('consDatRepCDEDGrid').getSelectionModel().selectAll();
		});
		var params=[];
...
var reportTemplatInfo = new Ext.form.FormPanel({
			...
			items : [{
						...
						items : [{
							...
							readOnly : false,
							labelSeparator : '',
							name : 'consDatRepCDEDDescription',
							anchor : '100%',
...
function templateManager() {
	Ext.getCmp('consDatRepCDEDGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("consDatRepCDEDType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("consDatRepCDEDName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("consDatRepCDEDDescription").getValue()
				},
...
reportParamsPanelSecond = new Ext.Panel({
			...
			items : [reportParamsPanel, {
				...
				items : [{
							xtype : 'grid',
							mode : 'remote',
							id : 'consDatRepCDEDGrid',
							name : 'consDatRepCDEDGrid',
							// ds : reportParamStore,
							region : 'south',
							cm : reportParamCm,
...
//							autoLoad : true,
				...
				tbar : [{
							xtype : 'label',
							html : "<font font-weight:bold;>用户日电量明细</font>"
						},'-', {
							xtype : "checkbox",
							boxLabel : "全选",
							id : "consDatRepCDED_selectAll",
							listeners : {
								check : function(c, v) {
									if (v) {
										reportParamStore.setAllSelect(true);
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('consDatRepCDED_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('consDatRepCDEDGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("consDatRepCDEDStartTime").getRawValue();
	var endTime = Ext.getCmp("consDatRepCDEDEndTime").getRawValue();
