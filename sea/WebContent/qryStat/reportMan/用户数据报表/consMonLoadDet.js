...
reportParamStore.on('load', function() {
			Ext.getCmp('consDatRepCMLDGrid').getSelectionModel().selectAll();
		});
		var params=[];
...
var reportTemplatInfo = new Ext.form.FormPanel({
			...
			items : [{
						...
						items : [{
							...
							readOnly : false,
							labelSeparator : '',
							name : 'consDatRepCMLDDescription',
							anchor : '100%',
...
function templateManager() {
	Ext.getCmp('consDatRepCMLDGrid').getStore().eachAll(function(key,v){
	...
	Ext.Ajax.request({
				...
				params : {
					...
					'reportType' : reportTemplatInfo.getForm()
							.findField("consDatRepCMLDType").getValue(),
					'reportName' : reportTemplatInfo.getForm()
							.findField("consDatRepCMLDName").getValue(),
					'consNoList' : consNoList,
					'reportParam' : reportTemplatInfo.getForm()
							.findField("consDatRepCMLDDescription").getValue()
				},
...
reportParamsPanelSecond = new Ext.Panel({
			...
			items : [reportParamsPanel, {
				...
				items : [{
							xtype : 'grid',
							mode : 'remote',
							id : 'consDatRepCMLDGrid',
							name : 'consDatRepCMLDGrid',
							// ds : reportParamStore,
							region : 'south',
							cm : reportParamCm,
...
//							autoLoad : true,
				...
				tbar : [{
					...
					width : 30,
					handler : function() {
						if (Ext.getCmp('consDatRepCMLD_selectAll').checked) {
...
// reportParamStore.setGrid(Ext.getCmp('consDatRepCMLDGrid'));
...
getParamsUrl = function() {
	...
	var startTime = Ext.getCmp("consDatRepCMLDStartTime").getRawValue();
	var endTime = Ext.getCmp("consDatRepCMLDEndTime").getRawValue();
