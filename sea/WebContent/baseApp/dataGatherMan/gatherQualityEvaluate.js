...
//终端类型store
var tmnlTypeCodeStore = new Ext.data.JsonStore({
	url : 'baseapp/gatherQualityEvaluate!queryTmnlType.action',
	fields : ['tmnlTypeCode','tmnlType'],
	root : 'tmnlTypeCodeList'
});

var tmnlTypeCodeCombox = new Ext.form.ComboBox({
	store : tmnlTypeCodeStore,
	displayField : 'tmnlType',
	valueField : 'tmnlTypeCode',
	name : 'tmnlTypeCodeCombox',
	anchor : '95%',
	editable : false,
	mode : 'remote',
	triggerAction : 'all',
	...
	items : [{
				...
				items : [{
							...
							handler:function(){
								...
								missingTerminalStore.baseParams = {
...
//采集失败终端 ColumnModel
var missingTerminalColumnModel = new Ext.grid.ColumnModel([
	new Ext.grid.RowNumberer(),{
		header : '供电单位',
		dataIndex : 'orgName',
		renderer:function(value){
			return '<div align = "left" ext:qtitle="供电单位" ext:qtip="' + value
				+ '">' + value + '</div>';
		}
	},{
		header : '用户编号',
		...
		dataIndex :'consName',
		renderer:function(value){
			return '<div align = "left" ext:qtitle="用户名称" ext:qtip="' + value
				+ '">' + value + '</div>';
		}
	},{
		header : '终端地址',
		...
		renderer:function(value){
			if(value == 1){
				return '<font color="green";font-weight:bold>终端在线</font>'
			}else{
				return '<font color="red";font-weight:bold>终端不在线</font>'
			}
		}
	}
]);	
...
//采集失败终端 store
var missingTerminalStore = new Ext.data.Store({
	url:'baseapp/gatherQualityEvaluate!queryMissingTerminal.action',
	reader:new Ext.data.JsonReader({
		root : 'missingTerminalList',
		totalProperty : 'totalCount'
	},[{
		name : 'orgName'
	},{
...
//采集失败终端 gridPanel
var missingTerminalGridPanel = new Ext.grid.GridPanel({
	region:'center',
	cm : missingTerminalColumnModel,
	store : missingTerminalStore,
...
Ext.onReady(function(){
			...
			var missingTerminalPanel = new Ext.Panel({
				title:'抄表失败列表',
				border:false,
				frame:false,
				layout :'border',
				items:[missingTerminalGridPanel]
			});
			
			var qualityTabPanel = new Ext.TabPanel({
				region : 'center',
				border:false,
				frame:false,
				activeItem : 0,
				items:[qualityViewPanel,missingTerminalPanel]
			});
			
			var qualityMainPanel = new Ext.Panel({
				border:false,
				frame:false,
				layout:'border',
				items:[qualityQueryPanel,qualityTabPanel]
			});
			
			renderModel(qualityMainPanel, '采集质量检查');
		});
