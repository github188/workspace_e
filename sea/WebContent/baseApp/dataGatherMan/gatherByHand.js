...
var orgType = '';
	...
	function clearData() {
		handRecordGrid.getStore().removeAll();
	}
...
/**grid----------------------------------------------------------------------------------**/			
	...
	var handCm = new Ext.grid.ColumnModel([handRowNumberer,handSm,
		...
		{header:'漏点时间',dataIndex:'statDate',align:'center',sortable:true, width:80,
		renderer : function(val) {
			return Date.parseDate(val,'Y-m-dTH:i:s').format('Y-m-d');
		}},
		{header:'测量点号',dataIndex:'mpSn',align:'center',sortable:true,width:70},
		{header:'电能表资产号',dataIndex:'mpNo',align:'center',sortable:true,width:110,
    		renderer : function(val) {
			var html = '<div align = "left" ext:qtitle="电能表资产号" ext:qtip="'
				+ val + '">' + val + '</div>';
			return html;}},
//		{header:'计量名称',dataIndex:'mpName',align:'center',sortable:true,width:80},
