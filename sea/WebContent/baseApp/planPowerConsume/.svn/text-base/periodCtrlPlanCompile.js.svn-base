	...
	var sdkmb1 = new Ext.Panel({
	  	...
	  	items:[{
				...
				items:[{
					xtype:'button',
					handler:function(){
						var templateName = Ext.getCmp("plan_powerPlanCompile_periodTemplateName").getValue().trim();
						if(templateName == "") {
							Ext.Msg.alert("提示","模板名称不能为空");
							return;
						}
						if(!mbcs_sdkmb.getForm().isValid()){
	...
	var sdkmbUpdate1 = new Ext.Panel({
	  	...
	  	items:[{
				...
				items:[{
					xtype:'button',
					handler:function(){
						var templateName = Ext.getCmp("plan_powerControl_sdkmbUpdate").getValue().trim();
						if(templateName == "") {
							Ext.Msg.alert("提示","模板名称不能为空");
							return;
						}
						if(!mbcs_sdkmbUpdate.getForm().isValid()){
							Ext.MessageBox.alert("提示","存在无效参数！");
				     	     return;
				     	}
						var templateId=templateNameComboBox.getValue();
						sdkmbUpdate3.find('name','template.templateId')[0].setValue(templateId);
...
//---------------------------------------时段控 panel 开始-----------------------------------------------------
	...
	planTemplateStore.on('load',function(s){
		templateNameComboBox.setValue(sdkTemplateId);
		if(null!=sdkTemplateId||""!=sdkTemplateId){
			planTemplate = s.getById(sdkTemplateId);
	...
	templateNameComboBox.on("collapse", function (o){
		...
		planTemplate = store.getById(o.getValue());
		if(planTemplate ==null ||planTemplate==''){
			return;
		}
	  	...
	  	items:[{
				...
				items:[{
					...
					handler:function(){
						...
						oldTemplateName=planTemplate.get('templateName');
						Ext.getCmp("plan_powerControl_sdkmbUpdate").setValue(oldTemplateName);
	...
	if(typeof(staticPeriodSchemeName) != 'undefined'){
		periodPanel.setTitle('方案名：'+staticPeriodSchemeName);
	}
	   	   ...
	   	   sdk_cszxStore.load({
				...
				callback: function(records, options, success){
	               if(null != records && 0 < records.length){
	            	    ...
	            	    executeCheckbox.setDisabled(false);
...
//			{header : '终端资产号', 	sortable:true, dataIndex:'tmnlAssetNo', align:'center',hidden:true},
	...
	var sdk_cszxGrid = new Ext.grid.GridPanel({
			...
			handler : function() {
				...
				//保存方案明细
     	    	Ext.Ajax.request({
  		     		url:'./baseapp/addPwrCtrlScheme!addPeriodCtrlScheme.action',
  		     		params : {
  		     			curveNo:curveNo,
  		     			sectionNo:sectionNo,
  		     			templateName:templateName,
  		     			pwrCtrlList:pwrCtrlList,
  		     			schemeId:staticPeriodSchemeId
  		     		},
  		     		success : function(){
  		     			Ext.MessageBox.alert("提示","方案保存成功");
  		     			Ext.getBody().unmask();
...
//						}],
    ...
    renderModel(periodRadio,'时段控方案编制');	
...
//  监听左边树点击事件
	...
	if(typeof(staticPeriodSchemeId) != 'undefined') {
		//planTemplateStore.load();
		sdk_cszxStore.removeAll();
   	    sdk_cszxStore.baseParams = {
   	   	   schemeId : staticPeriodSchemeId,
	...
	staticPeriodSchemeActivate=0;
	
	Ext.getCmp('时段控方案编制').on('activate', function() {
		if(staticPeriodSchemeActivate==1){
			if(typeof(staticPeriodSchemeName) != 'undefined'){
	 			periodPanel.setTitle('方案名：'+staticPeriodSchemeName);
	 		}
	 		if(typeof(staticPeriodSchemeId) != 'undefined'){
