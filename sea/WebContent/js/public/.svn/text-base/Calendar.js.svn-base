...
//农历年份
function lunarYear() {
	var nowYear = SY > 1900 ? SY : (SY + 1900);
	return (cyclical(nowYear - 1900 + 36));
}
//阳历年份
function year() {
	var yearNum = SY > 1900 ? SY : (SY + 1900);
	return (yearNum);
}
//阳历月份
function month() {
...
function YYMMDD() {
	var cl = '<font color="#000066" STYLE="font-size:9pt;">今天是 ';
	if (now.getDay() == 0)
...
function solarDay3() {
	...
	var sFtv = new Array("0101*元旦", "0214 情人节", "0308 妇女节", "0312 植树节",
			...
			"0601 儿童节", "0701 建党节 香港回归纪念", "0801 建军节", "0808 父亲节", "0910 教师节",
			"1001*国庆节", "1006 老人节", "1220 澳门回归纪念", "1225 圣诞节")
	...
	// 节气
	tmp1 = new Date((31556925974.7 * (SY - 1900) + sTermInfo[SM * 2 + 1]
			* 60000)
			+ Date.UTC(1900, 0, 6, 2, 5))
	tmp2 = tmp1.getUTCDate()
	if (tmp2 == SD)
		solarTerms = solarTerm[SM * 2 + 1]
	tmp1 = new Date((31556925974.7 * (SY - 1900) + sTermInfo[SM * 2] * 60000)
			+ Date.UTC(1900, 0, 6, 2, 5))
	tmp2 = tmp1.getUTCDate()
	if (tmp2 == SD)
	...
	if (solarTerms == '' && solarFestival == '' && lunarFestival == '')
		...
		festival = '<FONT COLOR="#000066" STYLE="font-size:9pt;">' + solarTerms
				+ ' ' + solarFestival + ' ' + lunarFestival + '</FONT>';
