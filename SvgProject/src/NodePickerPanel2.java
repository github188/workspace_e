...
public class NodePickerPanel2 extends JPanel implements ActionMap {
	 ...
	 */
	public NodePickerPanel2(NodePickerController controller) {
	...
	private void initialize() {
		...
		String isWellFormedLabelVal = resources
				.getString("IsWellFormedLabel.wellFormed");
	...
	private void updateViewAfterSvgInput(Element referentElement,
		...
		if (referentElement != null) {
			...
			String isWellFormedLabelVal = resources
					.getString("IsWellFormedLabel.notWellFormed");
			isWellFormedLabel.setText(isWellFormedLabelVal);
	 ...
	 */
	private void updateElementAttributes(Element element,
	 ...
	 * Node.lookupNamespaceURI method, starting from the parent element of the
	 * element being edited / created.
	 * 
	 * @param prefix
	 *            The given prefix
	...
	public void updateOnDocumentChange(String mutationEventType, Node targetNode) {
		if (mode == VIEW_MODE) {
			if (this.isShowing()
					&& shouldUpdate(mutationEventType, targetNode,
							getPreviewElement())) {
	...
	/**
	 * Parses the given xml and return parsed document's root element. Used to
	 * check whether the given xml is well formed.
	 * 
	 * @param xmlString
	 *            Xml as a String
	 * @return Element
	...
	private boolean isANodePickerComponent(Component component) {
		return SwingUtilities.getAncestorOfClass(NodePickerPanel2.class,
				component) != null;
	}
	...
	protected class NodePickerEditListener extends FocusAdapter {
		...
		/*
		 * public void focusLost(FocusEvent e) { // Prompts the user to save
		 * changes that he made for an element, // when the NodePicker loses
		 * focus if (!isANodePickerComponent(e.getOppositeComponent()) &&
		 * !e.isTemporary() && isDirty) { promptForChanges(); } }
		 */
	}
	...
	protected class XMLAreaListener implements DocumentListener {
		...
		private void updateNodePicker(DocumentEvent e) {
			if (getMode() == EDIT_MODE) {
				updateViewAfterSvgInput(parseXml(svgInputPanel.getNodeXmlArea()
						.getText()), clonedElement);
			} else if (getMode() == ADD_NEW_ELEMENT) {
				updateViewAfterSvgInput(parseXml(svgInputPanel.getNodeXmlArea()
						.getText()), previewElement);
			}
		}
	}
	...
	protected class AttributesTableModelListener implements TableModelListener {
		...
		private void updateNodePicker(TableModelEvent e) {
			if (getMode() == EDIT_MODE) {
				updateElementAttributes(clonedElement,
						(AttributesTableModel) (e.getSource()));
				updateNodeXmlArea(clonedElement);
			} else if (getMode() == ADD_NEW_ELEMENT) {
				updateElementAttributes(previewElement,
						(AttributesTableModel) (e.getSource()));
	...
	protected class ApplyButtonAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (getMode() == EDIT_MODE) {
				fireUpdateElement(new NodePickerEvent(NodePickerPanel2.this,
						xmlAreaText, previewElement,
						NodePickerEvent.EDIT_ELEMENT));
			} else if (getMode() == ADD_NEW_ELEMENT) {
				fireAddNewElement(new NodePickerEvent(NodePickerPanel2.this,
						xmlAreaText, parentElement,
			...
			try {
				...
				writer.write(EOL);
				svgdocument = (SVGDocument) svgApplet.getDocument();
				...
				root.setAttributeNS(XMLConstants.XML_NAMESPACE_URI, "xml:base",
						svgdocument.getURL());
			...
			//	String sql = "insert into svg_file_names(svgname,svgfile) values (?,?) ";
				String sql="update svg_file_names set svgfile=? where svgname=? ";
	 ...
	 * 
	 * @param event
	 *            The associated NodePickerEvent2 event
	 */
	...
	public static class NodePickerEvent extends EventObject {
		...
		/**
		 * Gets the context node. 'EDIT_ELEMENT' event type - the context node
		 * is the original element being previewed. 'ADD_NEW_ELEMENT' event type
		 * - the context node is the parent node of the element being added
		 * 
	...
	public static interface NodePickerListener extends EventListener {
		/**
		 * Updates the element from the data contained in the NodePickerEvent2.
		 */
		void updateElement(NodePickerEvent event);

		/**
		 * Adds the element from the data contained in the NodePickerEvent2.
		 */
	...
	public static class NameEditorDialog extends JDialog implements ActionMap {
		...
		protected void initialize() {
			this.setSize(resources.getInteger("Dialog.width"), resources
					.getInteger("Dialog.height"));
