...
public class CreatorPlus2 extends JFrame 
		implements ActionListener, ChangeListener {
	
  // Two Buttons to draw the shapes and a button to save 
  // them into a file
  private JButton btnCircle, btnSquare, btnSave; 
	
  // File Chooser to name and place the file being saved
  // in a file system
  JFileChooser fcSave = new JFileChooser();
	
  // Two Spinners to change the stroke width and the shapes'
  // size
  ...
  // The SVGDocument is used twice: first to create an
  // instance of SVG Generator and then to pass the created
  // SVG content to the JSVGCanvas
  private SVGDocument doc;
	
  // We shall use Spinners to change these values
  ...
  public CreatorPlus2() {
    ...
    super("SVG Creator Plus 2");

    this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    JPanel panel = new JPanel();
		
    // This is an auxilliary panel to group together 
    // Buttons, Labels and Spinners
    JPanel p = new JPanel();
    ...
    // And then the Spinners themselves are created 
    // and associated with a Change Listener - basic Swing
    SpinnerModel widthModel = new SpinnerNumberModel(20, 10, 50, 10);
    spStroke = new JSpinner(widthModel);
    spStroke.addChangeListener(this);		
    SpinnerModel sizeModel = new SpinnerNumberModel(90, 60, 110, 10);
    spSize = new JSpinner(sizeModel);
    spSize.addChangeListener(this);
		
    // All the controls are added to the auxilliary panel
    ...
    panel.add("South", btnSave);
    this.setContentPane(panel);
    this.pack();
    this.setBounds(150, 150, this.getWidth(), this.getHeight());
		
    // The SVGDocument and the SVG Generator are created 
    ...
    // There is no content at this stage, but placing this
    // method call here allows to avoid latency when drawing
    // the first shape
    passContent();
  }
  ...
  public static void main(String[] args) {
    CreatorPlus2 creator = new CreatorPlus2();
  ...
  public void actionPerformed(ActionEvent ae) {
    ...
    // Randomly changing the location of shapes
    ...
    if (source == btnCircle) {
      ...
      // Passing the created content to JSVGCanvas for
      // displaying
	passContent();
    }
    if (source == btnSquare) {
  ...
  private void passContent() {
    // This is where the generated content is passed 
    // to the JSVGCanvas
  ...
  private void drawSquare(int x, int y) {
    Rectangle rect = new Rectangle(x, y, figureSize, figureSize);
  ...
  private void drawCircle(int x, int y) {
    Ellipse2D circle = new Ellipse2D.Double(x, y, figureSize, figureSize);
