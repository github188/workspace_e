...
public class svgApplet extends Applet implements ActionListener {
	...
	boolean showWhitespace = true;// Dom-viewer閻ㄥ嫮鈹栭惂鑺ユ瀮閺堫剙灏�
	Panel domPanel = new Panel(); // Dom-viewer閻ㄥ埦anel
	...
	/*
	 * private void Domview_new() {
	 * 
	 * 
	 * domviewer = new DOMViewer(controller); domviewer.setVisible(true); if
	 * (domviewer != null) { if (domviewer.isVisible() && d != null) {
	 * domviewer.setDocument(d, (ViewCSS) d.getDocumentElement()); } }
	 * 
	 * }
	 */

	/*
	 * private void DomView() { JFrame jframe = new JFrame(); JButton
	 * closeButton = new JButton("CloseButton"); jframe.setSize(500, 500);
	 * 
	 * jframe.add(domPanel);
	 * 
	 * JPanel p = new JPanel(new BorderLayout());
	 * 
	 * JCheckBox cb = new JCheckBox("ShowWhitespaceCheckbox.text");
	 * 
	 * cb.setSelected(showWhitespace);
	 * 
	 * 
	 * cb.addItemListener(new ItemListener() { public void
	 * itemStateChanged(ItemEvent ie) { setShowWhitespace(ie.getStateChange() ==
	 * ItemEvent.SELECTED); } });
	 * 
	 * p.add(cb, BorderLayout.WEST); p.add(closeButton, BorderLayout.EAST);
	 * jframe.add(p, BorderLayout.SOUTH); // Document document =
	 * domViewerController.getDocument();
	 * 
	 * if (d != null) { // panel.setDocument(document, (ViewCSS) document //
	 * .getDocumentElement()); domPanel.setDocument(d, null); }
	 * 
	 * 
