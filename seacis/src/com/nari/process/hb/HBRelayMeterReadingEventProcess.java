...
public class HBRelayMeterReadingEventProcess implements Runnable{
	
	private static final Log log = LogFactory.getLog(HBRelayMeterReadingEventProcess.class);
	private EventBean[] newEvents;
	private EventBean[] oldEvents;
	public HBRelayMeterReadingEventProcess(EventBean[] newEvents,EventBean[] oldEvents){
	...
	public void run() {
		log.debug("【HBRelayMeterReadingEventProcess抄读电表开始......】");
		...
		try {
			...
			if ("1".equals(flowFlag)) {
				log.debug("----------------------------【HBRelayMeterReadingEventProcess集抄流程开始】----------------------------");
				...
				log.debug("----------------------------【HBRelayMeterReadingEventProcess集抄流程结束】----------------------------");
				
			} else {
				log.debug("----------------------------【HBRelayMeterReadingEventProcess负控流程开始】----------------------------");
				flowStatus = relayMeterReading(tmnlAssetNo);
				log.debug("----------------------------【HBRelayMeterReadingEventProcess负控流程结束】----------------------------");
			}
			...
			if ("1".equals(flowFlag)) {
				...
				log.debug("----------------------------【HBRelayMeterReadingEventProcess流程结束】----------------------------");
				//触发事件：下发终端测量点参数
				Constant.sendEvent(new HBSendTmnlMpParamEvent(appNo, terminalId, tmnlAssetNo, debugId, flowFlag, meterFlag));//TODU
			} else {
